...
#def main(dmin, dmax, maxSteps, sourcefile, rulesfile, topx, mwmax_source, mwmax_cof, outDir, timeout):
def main(dmin, dmax, maxSteps, sourcefile, topx, mwmax_source, mwmax_cof, outDir, timeout):
    try:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-resultsCSV', type=str)
    ...
    #exit_code = main(params.dmin, params.dmax, params.maxSteps, cp_source, cp_rules, params.topx, params.mwmax_source, params.mwmax_cof, outDir, float(params.timeout)*60.0) # input is in minutes and we convert to seconds
    exit_code = main(params.dmin, params.dmax, params.maxSteps, cp_source, params.topx, params.mwmax_source, params.mwmax_cof, outDir, float(params.timeout)*60.0) # input is in minutes and we convert to seconds
    ...
    csvResults = glob.glob(outDir+'/results.csv')
    try:
        shutil.copy2(csvResults[0], params.resultsCSV)
    except IndexError:
