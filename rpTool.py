...
def run_rp2(source_bytes, sink_bytes, rules_bytes, max_steps, topx=100, dmin=0, dmax=1000, mwmax_source=1000, mwmax_cof=1000, timeout=30, ram_limit=None, partial_retro=False):
    ...
    with tempfile.TemporaryDirectory() as tmp_output_folder:
        sink_path = tmp_output_folder+'/tmp_sink.csv'
        with open(sink_path, 'wb') as outfi:
            outfi.write(sink_bytes)
        source_path = tmp_output_folder+'/tmp_source.csv'
        with open(source_path, 'wb') as outfi:
            outfi.write(source_bytes)
        #rulesfile, fname_rules = readCopyFile(rulesfile, tmp_output_folder)
        rules_path = tmp_output_folder+'/tmp_rules.csv'
        ...
        try:
            ...
            try:
                count = 0
                with open(tmp_output_folder+'/results.csv') as f:
            ...
            try:
                count = 0
                with open(tmp_output_folder+'/source-in-sink.csv') as f:
                    reader = csv.reader(f, delimiter=',', quotechar='"')
                    for i in reader:
                        count += 1
                if count>1:
            ...
            if is_time_out:
                if not is_results_empty and partial_retro:
                    ...
                    with open(tmp_output_folder+'/results.csv', 'rb') as op:
                        results_csv = op.read()
